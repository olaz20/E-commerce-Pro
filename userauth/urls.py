...
urlpatterns = [
    ...
    path('auth/request-password-email/', RequestPasswordEmail.as_view(), name='request-password-email'),
    path('auth/login/', LoginView.as_view(), name='login'),
    path('auth/logout/', LogoutView.as_view(), name='logout'),
    
    # Password reset-related routes
    path('auth/password-reset/<uidb64>/<token>/', PasswordTokenCheckAPI.as_view(), name='password-reset'),
    path('auth/set-new-password/', SetNewPasswordAPIView.as_view(), name='set-new-password'),
    path('auth/validate-otp-and-reset-password/', ValidateOTPAndResetPassword.as_view(), name='validate-otp-and-reset-password'),
