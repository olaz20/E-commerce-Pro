    ...
    def handle(self, *args, **kwargs):
        ...
        with open(file_path, mode="r") as file:
            ...
            for row in reader:
                ...
                # Get or create LGA
                lga, _ = LocalGovernment.objects.get_or_create(
                    name=row["lga"], state=state
                )

                shipping_fee, created = ShippingFee.objects.get_or_create(
                    lga=lga, defaults={"fee": row["shipping_fee"]}
                )
                ...
                self.stdout.write(
                    f"Added/Updated {lga.name} with shipping fee {shipping_fee.fee}"
                )
